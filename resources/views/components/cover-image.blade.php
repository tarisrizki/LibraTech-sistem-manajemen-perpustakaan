...
@elseif($olUrl)
    <img
        ...
        decoding="async"
        fetchpriority="{{ $fetchPriority }}"
        onerror="if(this.dataset.triedId!=='1' && '{{ $idUrl }}'){this.dataset.triedId='1';this.src='{{ $idUrl }}';}else{this.style.display='none';var f=this.nextElementSibling; if(f){f.style.display='grid'; f.classList.remove('hidden');} }"
